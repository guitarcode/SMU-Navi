...
public class GetBusAndSubwayAPI {
    ...
    public static void main(String[] args) throws ParserConfigurationException, IOException, SAXException {
        ...
        List<PathInfo> pathInfo = getBusAndSubwayAPI.callApi(new GpsPoint("126.911128", "37.6062954"));
     ...
     * @throws SAXException
     */
    public List<PathInfo> callApi(GpsPoint startPoint) throws ParserConfigurationException, IOException, SAXException {
            ...
            Node iNode = itemList.item(0);

            Element iElement = (Element) iNode;

            NodeList pathList = iElement.getElementsByTagName("pathList");

            List<TransportInfo> transitInfoList = new ArrayList<>();
            ...
            for (int i = 0; i < pathList.getLength(); i++) {
                ...
                if(pElement.hasAttribute("routeId")){
                    String busId = ApiUtilMethod.getTagValue("routeId", pElement);
                    transitInfoList.add(new TransportInfo(TRANSIT.BUS, busNum, from, to));
                    busRouteListAPI.getRouteList(gpsPointList, busId, from, to);
                }
                else{
                    //지하철 경로일 경우 코드가 들어올 부분
                    transitInfoList.add(new TransportInfo(TRANSIT.SUBWAY, busNum, from, to));
                }
