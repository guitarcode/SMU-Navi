...
import smu.poodle.smnavi.domain.BusRouteInfo;
import smu.poodle.smnavi.domain.Route;
import smu.poodle.smnavi.errorcode.ExternApiErrorCode;
import smu.poodle.smnavi.externapi.*;
import smu.poodle.smnavi.repository.TransitRepository;

import java.util.ArrayList;
import java.util.List;

import static smu.poodle.smnavi.externapi.ApiUtilMethod.callXmlApi;
import static smu.poodle.smnavi.externapi.ApiUtilMethod.getTagValue;
...
public class BusInfoApi {
    ...
    public List<BusRouteInfo> parsingBusPosInfo(Document xmlContent, int time, Route route){
        ...
        for (int i = 0; i < itemList.getLength(); i++) {
            ...
            int congetion = Integer.parseInt(getTagValue("congetion", element));
            boolean isFull = getTagValue("isFullFlag", element).equals("1");
